...
# --path_dir "D:\\Песочница\\Робот ИНЖИ (КОМПАС-3D)"
...
def parse_kompas_files(paths, new_path_to_dir):
    ...
    for path in paths:
        ...
        if path.endswith(".a3d"):
            ...
            save_CDW(kompas6_constants, kompas_object, application_api7, path, new_path_to_dir)
...
def save_CDW(kompas6_constants, kompas_object, application_api7, filePath_3D, new_path_to_dir):
    ...
    if filePath_3D.endswith('.a3d'):
        ...
        cdw_doc_IKompasDocument.SaveAs(new_path_to_dir + os.path.basename(new_path))
    # Если модель
...
if __name__ == "__main__":
    ...
    list_data_from_3D = parse_kompas_files(filenames, args.path_dir)
    if len(list_data_from_3D) > 0:
        print_to_excel(list_data_from_3D, args.path_dir)
